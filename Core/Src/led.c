...
void LED_Init(){
	...
	leds[1].port = BLUE_LED_PORT;
	leds[1].pin = BLUE_LED_PIN;
	leds[1].enabled = true;

	leds[2].port = BUZZER_PORT;
	leds[2].pin = BUZZER_PIN;
	leds[2].enabled = true;
}
...
void LED_Update(void)
{
    ...
    for (uint8_t i = 0; i < MAX_LEDS; i++) {
		...
		if (leds[i].state == 1) {
			...
			// OFF phase
			if (delta < step.off_ms) continue;

			LedStep_t next = leds[i].pattern[leds[i].stepIndex + 1];
			...
			if (next.on_ms == 0 && next.off_ms == 0) {
				...
				if (leds[i].repeatCount > 0 &&
					leds[i].currentRepeat >= leds[i].repeatCount) {
