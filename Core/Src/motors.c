...
typedef enum {
	...
	MOTOR_STATE_WAIT2,
	MOTOR_STATE_GO_FORWARD,
	MOTOR_STATE_GO_BACKWARD,
	MOTOR_STATE_STOP
} MotorState_t;
...
void MOTOR_GoForwardAndBackward(CallbackF f){
	callbackf = f;
	motorState = MOTOR_STATE_WAIT2;
	duration = 500;
	startTime = HAL_GetTick();
}
...
void MOTOR_Update(){
	...
	switch(motorState)
	{
		...
		// ENCODER TEST
		case MOTOR_STATE_WAIT2:
			if(delta >= duration){
				motorState = MOTOR_STATE_GO_FORWARD;
				startTime = HAL_GetTick();
				MOTOR_Set(DIR_FORWARD, CALIBRATING_SPEED, 500);
			}
			break;

		case MOTOR_STATE_GO_FORWARD:
			if(delta >= duration){
				motorState = MOTOR_STATE_GO_BACKWARD;
				startTime = HAL_GetTick();
				MOTOR_Set(DIR_BACKWARD, CALIBRATING_SPEED, 500);
			}
			break;

		case MOTOR_STATE_GO_BACKWARD:
			if(delta >= duration){
				motorState = MOTOR_STATE_STOP;
			}
			break;
